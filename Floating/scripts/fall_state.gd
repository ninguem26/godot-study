...
export(NodePath) var float_node
...
onready var float_state: BaseState = get_node(float_node)
...
func enter() -> void:
	...
	elif [float_state].has(player.state_machine.previous_state):
		current_fall_speed = 150
	else:
